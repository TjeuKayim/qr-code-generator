# https://github.com/kantlove/csblocks
...
def char_for(code, encoding):
  ...
  """
  return chr(unicode_offset | char_map[code]) #.encode(encoding)
...
class Grid:
  ...
  def unset(self, y, x):
    ...
    if self.cols <= x or x < 0 or y < 0 or len(self.grid) <= r:
      return
  ...
  def expand(self, new_rows):
    ...
    for i in range(new_rows - old):
      self.grid.append(array_init(self.cols, 0))
